...
To update ${APP_NAME} itself, use the package manager that installed it.
...
export async function handlePackageCommand(
	...
	if (options.command === "update" && !options.updateTarget) {
		console.log(`Update ${APP_NAME} with the package manager that installed it.`);
		console.log(
			chalk.dim(
				`Use "${APP_NAME} update --extensions" to update installed packages or "${APP_NAME} update --models" to refresh model catalogs.`,
			),
		);
		return true;
	}
	...
	try {
		switch (options.command) {
			...
			case "update": {
				// Models and no-target updates are handled before settings resolution;
				// only extension updates reach this point.
